...
def model(loops,seed,n_estimators=100,kn=5,test_size=0.2):
  '''
  inputs:
    loops: iteration number
  ...
  #root
  filepath = "C:\Users\Xuan Kai\studyforrest_dFC\model_timeseries"
  ...
  #load data
  df = pd.read_csv("C:\Users\Xuan Kai\studyforrest_dFC\timeseries_data_min10.csv",index_col=0)
  print('data loaded.')
  ...
  try:
    ...
    fo.write('%d,%d,%s,%s,%s,%d\n' % (loops,seed,round(baseline_acc,4),round(baseline_precision,4),round(baseline_recall,4),baseline.best_ntree_limit))
    fo.close()
    
    f = open(classrep_path + '\baseline_split%d.txt' % loops,"w+")
    ...
    print('run %d accuracy: ' % loops + str(baseline_acc))
